...
# IE = inflect.engine()
IE = None
ASSETS_PATH = resources.files("src.reward.assets")
...
def _load_lines(path):
    ...
    `src/reward/assets` directory for a file named `path`.
    """
    if not os.path.exists(path):
        newpath = ASSETS_PATH.joinpath(path)
    if not os.path.exists(newpath):
        raise FileNotFoundError(f"Could not find {path} or src.reward.assets/{path}")
    path = newpath
