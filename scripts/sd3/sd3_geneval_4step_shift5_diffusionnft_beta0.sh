#!/bin/bash
#SBATCH --job-name=sd3_gen_4step_sh5_nft_b0
...
# SD3.5-medium on GenEval, 4-step DPM2 sampling with flow-matching shift=5.0,
# trained with the DiffusionNFT contrastive flow-matching loss
# (METHOD=diffusionnft). NO Girsanov KL anchor (BETA=0); contrastive guidance
# strength gamma = DIFFUSIONNFT_BETA = 0.1. ADV stays "diffusionnft" because the
# loss requires the optimality probability r in [0,1] from that advantage branch
# (adv=standard would feed out-of-[0,1] values into the convex r/(1-r) mixing and
# is mathematically incompatible). LR=1e-6, xt sampled fresh (XT_FROM_LATENTS=0).
# Config sets num_steps=4 / shift=5.0; guidance held at 1.0 (no CFG).
export NUM_GPUS=8
# MIN_GPUS=6 (not 4): the K-repeat sampler needs num_replicas*bsz % k(=24) == 0.
# 8gpu->bsz9->72, 6gpu->bsz8->48 both divide by 24; 4gpu->bsz8->32 does NOT and
# crashes (AssertionError "k can not div n*b"). So forbid 4-GPU allocations.
export MIN_GPUS=6
export ALLOW_GPU_6=true
export TASK="geneval"
export MAX_EPOCHS=10000
export METHOD="diffusionnft"
export ADV="diffusionnft"
export KL_METHOD="girsanov"
export DIFFUSIONNFT_BETA=0.1
export BETA=0
export ALPHA=0.0001
export SCALE=1.0
...
srun torchrun --master_port="${MASTER_PORT}" --nproc_per_node="${NUM_GPUS}" src/train.py \
    --config config/sd3.py:sd3_geneval_diffusionnft_4step_shift5
