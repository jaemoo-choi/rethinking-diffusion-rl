#!/bin/bash
#SBATCH --job-name=sd3_gen_4step_sh5_pepg_b0
...
# trained with the PEPG no-ratio policy objective (METHOD=pepg_noratio) using the
# STANDARD group-relative advantage (ADV=standard => A = SCALE*(R-mean)/std),
# NO KL regularization (BETA=0), LR=1e-6, and xt sampled fresh (NOT from the
# sampler trajectory: XT_FROM_LATENTS=0). Config sets num_steps=4 / shift=5.0;
# guidance held at 1.0 (no CFG).
export NUM_GPUS=8
...
srun torchrun --master_port="${MASTER_PORT}" --nproc_per_node="${NUM_GPUS}" src/train.py \
    --config config/sd3.py:sd3_geneval_4step_shift5
